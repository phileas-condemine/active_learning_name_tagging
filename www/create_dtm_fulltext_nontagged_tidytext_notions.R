...
save(list="tags",file="tags.RData")
...
# keep_ngrams=train_ngrams%>%dplyr::select(ngram,ngram_num)%>%distinct()%>%arrange(ngram_num)
...
tags_ngrams_to_add <- matching_indic_ngrams_dup%>%
  dplyr::select(i,tag_id)%>%distinct()%>%
  merge(matching_tags_ngrams,by.x="tag_id",by.y="i")



nb_features=max(tags_ngrams_to_add$j)
...
system.time(match_notions_indic_untagged <- do.call("rbind",sapply(1:nrow(notions),
                                                   function(i){
                                                     matching=str_which(pattern = paste0("(^|( ))(",notions$termes[i],")(( )|($))"),
                                                                        string = data2$Indicateur_enriched)
                                                     if(length(matching)>0){
                                                       data.frame(i= matching,
                                                                  tag_id=notions$tag_id[i],
                                                                  j=nb_features+1)
                                                     } else NULL
                                                   }
)))
match_notions_indic_untagged <- unique(match_notions_indic_untagged)

matching_indic_ngrams_dup <- rbind(matching_indic_ngrams_dup,
                                   tags_ngrams_to_add,
                                   match_notions_indic_untagged)
